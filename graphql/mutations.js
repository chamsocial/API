const {
  GraphQLObjectType, GraphQLString, GraphQLNonNull, GraphQLInt,
} = require('graphql')
const {
  Post, User, Comment, Activation,
} = require('../models')
  ...
  fields: {
    ...
    createUser: {
      ...
      resolve: (_, { username, email, password }, { ctx }) => (
        createUser({ username, email, password }, ctx.request.ip)
    ...
    activateUser: {
      ...
      resolve: (_, { code }, { ctx }) => (
        Activation
          .findOne({ where: { code, verified_at: null } })
          .then(activation => {
          ...
          .then(activation => User.findOne({ where: { id: activation.user_id } }))
          .then(user => {
            user.activated = 1
            return user.save()
          })
