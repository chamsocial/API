...
describe('Login', () => {
  ...
  it('should respond with an error', () => request
    .post('/v2/login')
  ...
  it('should fail with invalid password', () => request
    .post('/v2/login')
  ...
  it('should succeed with valid email & pasword', () => request
    .post('/v2/login')
  ...
  it('should also succeed with valid username', () => request
    .post('/v2/login')
  ...
  it('should return an object containing a accessToken', () => request
    .post('/v2/login')
    .send({ username: user.username, password: user.password })
    .then(res => {
  ...
  it('should return an object containing a accessToken', () => {
    expect(dbUser.last_login).to.be.undefined
    return request
      .post('/v2/login')
      .send({ username: user.username, password: user.password })
      .then(async () => {
        const loggedInUser = await User.findByPk(dbUser.id)
        expect(loggedInUser.last_login).to.not.be.undefined
      })
  })
})
